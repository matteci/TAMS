...
#SBATCH -t 01-15:00:00          # time in dd-hh:mm:ss you want to reserve for the job
...
resty=2015
initblock=2015        # block è periodo lungo come resampling. 
endblock=2100        # ultimo blocco: ti definisce lunghezza integrazione
co2_file="co2_sector0_values_rcp26_to2500.txt"
...
#CO2
co2=399.95 #not really updated here, just an indication

# TAMS Parameters
nc=26  #number of LEVES -thus less than traj- discarded at each iteration. 
targetstate=9.4 #Sv
Ktmax=3 #number of max iteration before stopping. This is due only to sbatch capcity. NB: next time you re-run you have to change starting prob!
wp=1 #starting prob
ktinit=0 #starting iteration of Tams, first iter =0
...
extractname='extract_score_rcptams' # estrae score
...
#      srun --mpi=pmi2 -K1 --resv-ports --exclusive --nodes=1 --ntasks=${nparallel} --mem=${mem} ${modelname} & # fai partire modello
    ...
    while [ ${traj} -le ${endtraj} ]
    do
	./${extractname} ${expdir} ${expname} ${traj} ${block} ${varname} ${domain} ${targetstate} ${resty} & # sottomette estrazione variabile 
